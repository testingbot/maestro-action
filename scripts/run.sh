...
[[ -n "${INPUT_THROTTLE_NETWORK:-}" ]] && args+=(--throttle-network "$INPUT_THROTTLE_NETWORK")
[[ -n "${INPUT_GEO_COUNTRY_CODE:-}" ]] && args+=(--geo-country-code "$INPUT_GEO_COUNTRY_CODE")
is_true "${INPUT_TUNNEL:-}" && args+=(--tunnel)
[[ -n "${INPUT_TUNNEL_IDENTIFIER:-}" ]] && args+=(--tunnel-identifier "$INPUT_TUNNEL_IDENTIFIER")
is_true "${INPUT_IGNORE_CHECKSUM_CHECK:-}" && args+=(--ignore-checksum-check)
